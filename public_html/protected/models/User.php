...
class User extends Model {
		...
		if ($this->isValid('login')) {
			...
			if($user == null)
				...
				if ($verify){
					...
					$userId = UserID::newUser($id, $username);
					return $userId;
				}
			}
			return false;
	...
	public function save() {
		if ($this->isValid('create')) {
		    ...
		    $sql = "INSERT INTO {$this->_tableName} (username, name, email, encrypted_password, phone, address, city, zip, card, created, updated, status)
		    		VALUES (:username, :name, :email, :encrypted_password, :phone, :address, :city, :zip, :card, now(), now(), :status)";
	...
	public function update() {
		if ($this->isValid('update')){
			...
			if($this->password != '') {
				$this->encrypted_password = password_hash($this->password, PASSWORD_BCRYPT);
				$password_sql = ", encrypted_password = :encrypted_password";
			}

		    // set the PDO error mode to exception
		    $this->_pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		    $sql = "UPDATE {$this->_tableName}
		    		SET username = :username, name = :name, email = :email".$password_sql.", phone = :phone, address = :address, city =:city, zip = :zip, card = :card, updated = now() , status = :status
		    		WHERE id = :id";
