				   		<form id="main-form" action="<?php echo $this->getConfig('base_url'); ?>/categoria<?php echo !empty($category->id)?"/{$category->id}":"";?>" method="<?php echo $method;?>">
			    			...
			    			<div class="form-group">
			    				...
			    				<label for="status-enabled"><input type="radio" class="form-control" name="status" id="status-enabled" value="1" <?php echo (isset($category->status) && $category->status == '1')?'checked':'';?>> Actiu</label>
			    				<label for="status-disabled"><input type="radio" class="form-control" name="status" id="status-disabled" value="0" <?php echo (isset($category->status) && $category->status == '0')?'checked':'';?>> Cancel·lat</label>
			    			</div>
			    			<?php endif; ?>
