...
class Product extends Model {
	...
	protected function _validate($scenario) {
		...
		// validate supplier_id: the value must exist in the database
		if (in_array($scenario, ['update']) && preg_match('pattern', $this->supplier_id)) {
			$inUse = $this->_isInUse('supplier_id', $this->supplier_id);
            if(!$inUse)
                $this->_addError('supplier_id', "No existeix un suministrador registrat amb aquest supplier_id.");
            elseif($inUse === false)
                $this->_addError('supplier_id', "Error comprobando supplier_id.");
		}
	...
	public function save() {
		if ($this->isValid()) {
		    ...
		    $sql = "INSERT INTO {$this->_tableName} (category_id, supplier_id, name, intro, description, price, stock, picture, created, updated, status) 
		    		VALUES (:category_id, :supplier_id, :name, :intro, :description, :price, :stock, :picture, now(), now(), :status)";
	...
	public function update() {
		if ($this->isValid()) {
		    ...
		    $sql = "UPDATE {$this->_tableName} 
		    		SET category_id = :category_id, supplier_id = :supplier_id, name = :name, intro = :intro, description = :description, price = :price, stock = :stock, picture =:picture, updated = now(), status = :status
		    		WHERE id = :id";

		    $stmt = $this->_pdo->prepare($sql);
		    $stmt->bindParam(':category_id', $this->category_id, PDO::PARAM_STR); 
		    $stmt->bindParam(':supplier_id', $this->supplier_id, PDO::PARAM_STR); 
	...
	public function findAll($conditions = null) {
		...
		try {
		    // set the PDO error mode to exception
		    $this->_pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		    $sql = "Select p.*, c.name as category, c.alias from ".$this->_tableName." p,categories c WHERE `category_id` = c.`id`".$condString;
